# BioMechAI Accuracy Fix — PHASE 2 of 5 ONLY
...
**This phase does NOT need the `KAGGLE_SESSION_SETUP.md` cell — it only reads an existing
`result.pkl` file with plain Python's `pickle` module, no MMAction2/GPU dependency at all.
Skip the setup cell for this phase specifically.**

**This phase does not depend on Phase 1's result — it can be run independently.**

---

## Step 0 — Confirm the input file exists (read-only)

```python
import os
print(os.path.exists('<path to the original epoch_18 result.pkl>'))
```
If `False`, STOP per Rule 1 and report — do not regenerate the file by re-running an
evaluation; ask the user where it actually is first.

## Your actual task for this phase: diagnose jumping_jack's clean-subset performance

```python
import pickle
with open('<path to result.pkl>', 'rb') as f:
    results = pickle.load(f)

bad_videos = {'jumping_jack_01_09', 'jumping_jack_01_12', 'jumping_jack_b2_17'}
clean_jj = [r for r in results if r['video_id'] not in bad_videos and r['true'] == 'jumping_jack']
clean_correct = sum(1 for r in clean_jj if r['pred'] == 'jumping_jack')
print(f"Clean jumping_jack clips: {len(clean_jj)}, correct: {clean_correct}, "
      f"recall: {clean_correct/len(clean_jj)*100:.1f}%")

from collections import Counter
print("Clean jumping_jack predictions:", Counter(r['pred'] for r in clean_jj))
```

If the file structure doesn't match, inspect one entry first (`print(results[0])`) and adapt
field names — but if fundamentally different or missing needed fields, STOP per Rule 1.

## Required output format (then stop)

- Clean-subset jumping_jack recall (the real printed number).
- What the clean clips are being predicted as instead, if recall is still low.
- One explicit sentence: is the frame-truncation explanation **complete** or **incomplete**?
