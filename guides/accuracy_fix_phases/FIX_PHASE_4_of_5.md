# BioMechAI Accuracy Fix — PHASE 4 of 5 ONLY

**Do not run this phase unless the user has explicitly told you to proceed after reviewing
Phase 3's prepared fixes. Retraining spends real GPU-hours on a limited monthly budget.**

**Do only what is described below. When finished, output your results in the exact format
requested at the end, then STOP.**
...
**Prerequisite: Phase 3's approved config changes must exist. If not, STOP and report.**

---

## Step 0 — Run the Kaggle session setup cell (mandatory, do this first, every time)

Run the complete `KAGGLE_SESSION_SETUP.md` cell now, even if this is a new session that
followed immediately after Phase 3 — do not assume the environment carried over unless you
are certain this is the exact same still-running kernel session with no restart in between.
Confirm its final verification printed successfully before proceeding.

**If it errors, STOP per Rule 1 — do not install, patch, or rebuild anything beyond what the
setup cell already specifies.**

## Your task for THIS phase only: retrain once, using the exact same launch method as before

Use the identical training invocation pattern, checkpoint interval, and Drive-sync hook setup
already proven working in the original production run — change only what Phase 3 approved
(the config file's content), not the launch method.

**Reuse the exact same `custom_dataset_train.pkl`/`custom_dataset_val.pkl` split.**

Track training accuracy every epoch alongside validation accuracy this time.

**If training fails to start, or errors partway through: STOP per Rule 1. Report the exact
error. Do not attempt fixes, package installs, or alternate launch methods on your own
initiative.**

## Required output format (then stop)

- Confirmation the setup cell completed successfully.
- Confirmation the same train/val split was reused (video overlap re-verified as 0).
- The full per-epoch loss, training accuracy, and validation accuracy log.
- If training accuracy diverges sharply from validation again, note at which epoch.
- Confirmation of which epoch produced the best validation checkpoint.
- Confirmation checkpoints synced to Drive successfully throughout.
