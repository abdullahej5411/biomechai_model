"""
BioMechAI Backlog Extraction — PHASE 2 of 5
- Pilot extracts landmarks for squat (73 candidates) and lunge (87 candidates)
- Duration-aware clip count: max(3, min(15, round(duration_seconds / 90)))
- 90 frames per clip (3 seconds at 30 FPS)
- Motion-sanity filter: threshold = 20% of v3 class median (squat: 0.001463, lunge: 0.001726)
- Saves extracted clips in data/landmarks_backlog/{exercise}/
- Logs extraction metrics in backlog_phase2_extraction_log.json
"""
...
LOG_OUT_FILE    = os.path.join(BASE_DIR, 'backlog_phase2_extraction_log.json')
...
print("Starting Phase 2 Pilot Landmark Extraction (squat & lunge only)...\n")

extraction_results = {}
summary_totals = {}

for ex in PILOT_EXERCISES:
...
print("=" * 90)
print("PHASE 2 REQUIRED OUTPUT: PER-VIDEO EXTRACTION TABLE")
print("=" * 90)

for ex in PILOT_EXERCISES:
    print(f"\n--- {ex.upper()} CANDIDATES ({len(extraction_results[ex])} videos) ---")
    print(f"| {'Video Filename':<28} | {'Duration':<12} | {'Target Clips':<14} | {'Clips Extracted':<17} | {'Clips Discarded':<17} |")
    print(f"|{'-'*30}|{'-'*14}|{'-'*16}|{'-'*19}|{'-'*19}|")
    for row in extraction_results[ex]:
        print(f"| {row['filename']:<28} | {row['duration_seconds']:>8.1f} s   | {row['target_clips']:>12d}   | {row['clips_extracted']:>15d}   | {row['clips_discarded']:>15d}   |")

print("\n" + "=" * 90)
print("PHASE 2 REQUIRED OUTPUT: PER-EXERCISE TOTALS")
print("=" * 90)
