# BioMechAI Backlog Extraction — PHASE 4 of 5 ONLY

**Do not run this phase unless the user has explicitly told you to proceed after reviewing
Phase 3's result. This file being given to you is not, by itself, sufficient authorization
if the user has not separately confirmed they reviewed Phase 3's numbers and want to
continue — if there is any ambiguity about that, ask before proceeding.**
...
**Prerequisite: Phase 1's `backlog_clean_deduped_candidates.json` and Phase 3's pilot
results must exist. If either is missing, stop and tell the user which earlier phase needs
to be run first.**

---

## Your task for THIS phase only: full extraction for the remaining 5 exercises

Same method as Phase 2 (duration-aware clip count, mandatory motion filter), applied now to
**bicep_curl, high_knees, jumping_jack, plank, and pushup** from Phase 1's deduplicated
candidate list. Squat and lunge were already done in Phase 2 — do not re-extract them here.
...
Motion filter: same as Phase 2, compared against each exercise's own current class median
(from existing v3 data).

**Do not merge, evaluate, or retrain anything in this phase — that's Phase 5. Only extract.**

## Required output format (then stop)

- Per-video: duration, clip count used, clips extracted, clips discarded by motion filter —
  for every candidate processed, across all 5 exercises.
